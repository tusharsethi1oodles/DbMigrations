...
# conn_db1 = connect_db("dmscopy")  # Source DB
# conn_db2 = connect_db("entitysample")  # Target DB

# # Load data from global_organisation where parent_organisation_id is NULL
# query = """
# SELECT organisation_name, creator_ledger_id, created_by, updated_by, created_at, updated_at, deleted_at
# FROM global_organisations
# WHERE parent_organisation_id IS NULL
# """

# # Use pandas to load the data into a DataFrame
# df = pd.read_sql(query, conn_db1)

# # Replace NaN values with None (NULL for MySQL)
# df = df.applymap(lambda x: None if pd.isna(x) else x)

# # Prepare the insert query for the entity table
# insert_query = """
# INSERT INTO entity (name, creator_ledger_id, created_by, updated_by, created_at, updated_at, deleted_at, type)
...
# # Iterate through the DataFrame and insert the data into the entity table
# for index, row in df.iterrows():
#     cursor_db2.execute(insert_query, (
#         row['organisation_name'], row['creator_ledger_id'], row['created_by'],
#         row['updated_by'], row['created_at'], row['updated_at'], row['deleted_at']
#     ))
